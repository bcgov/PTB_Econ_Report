...
fig0_caption <- function(text) {
  fig_counter <<- fig_counter + 1
  sprintf("Table %d: %s", fig_counter, text)
}
```
...
In `r end_label`, `r region_name` Regional District recorded `r metric("TRIP_VOLUME","TNS","last_value_fmt")` TNS trips. Compared with `r prev_label`, TNS trip volume was `r metric("TRIP_VOLUME","TNS","yoy_fmt")` (YoY). Year-to-date through `r end_label`, TNS trip volume was `r metric("TRIP_VOLUME","TNS","ytd_yoy_fmt")` relative to the same period in `r prev_year`. These results indicate strong momentum in demand for TNS services in the region.
