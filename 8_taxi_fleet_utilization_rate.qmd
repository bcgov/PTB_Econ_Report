...
In `r region_name` Regional District, taxi fleet utilization rate in `r end_label` was 
`r metric("FLEET_UTILIZATION_RATE","TAXI","last_value_fmt")`, 
compared with 
`r prev_metric("FLEET_UTILIZATION_RATE","TAXI")` 
in `r prev_month_label`, representing 
`r change_phrase(metric("FLEET_UTILIZATION_RATE","TAXI","mom_last"))` month over month. 
`r shift_narrative("FLEET_UTILIZATION_RATE","TAXI")`

On a year-over-year basis, taxi fleet utilization rate in `r end_label` was 
`r metric("FLEET_UTILIZATION_RATE","TAXI","last_value_fmt")`, 
compared with 
`r prev_year_metric("FLEET_UTILIZATION_RATE","TAXI")` 
in `r prev_label`, representing 
`r change_phrase(metric("FLEET_UTILIZATION_RATE","TAXI","yoy_last"))` year over year.

On a year-to-date basis, the average taxi fleet utilization rate from January to `r end_label` was 
`r format(round(ytd_metric("FLEET_UTILIZATION_RATE","TAXI", field="value", agg="avg"), 1), nsmall = 1)` , 
`r change_phrase(metric("FLEET_UTILIZATION_RATE","TAXI","ytd_yoy"))` from 
`r format(round(ytd_metric("FLEET_UTILIZATION_RATE","TAXI", field="value", agg="avg", year_offset=-1), 1), nsmall = 1)` 
over the same period (January to `r prev_label`). 
`r trend_narrative("FLEET_UTILIZATION_RATE","TAXI")`
