...
make_new_table(
  data      = raw_norm,
  indicator = "FLEET_UTILIZATION_RATE_RATE",
  region    = region_name,
  date_from = date_from,
  date_to   = date_to,
  font_size     = 8,
...
data       = raw_norm,
indicator  = "FLEET_UTILIZATION_RATE_RATE",  # Adjust indicator if needed
region     = region_name,
services   = c("TAXI", "TNS"),
area_type  = "REGIONAL",
date_from  = date_from,
...
In `r end_label`, `r region_name` Regional District recorded a total of `r metric("FLEET_UTILIZATION_RATE","TAXI","last_value_fmt")` in taxi fleet utilization rate  trips. On a year-over-year (YoY) basis, the taxi fleet utilization rate  trips was `r metric("FLEET_UTILIZATION_RATE","TAXI","yoy_fmt")`while on a year-to-date (YTD) basis it was `r metric("FLEET_UTILIZATION_RATE","TAXI","ytd_yoy_fmt")` relative to the same period in `r prev_year`. On a month-over-month (MoM) basis, taxi fleet utilization changed by `r metric("FLEET_UTILIZATION_RATE","TAXI","mom_fmt")`. `r trend_narrative("FLEET_UTILIZATION_RATE", "TAXI")` `r shift_narrative("FLEET_UTILIZATION_RATE", "TAXI")`

In `r end_label`, the `r region_name` Regional District recorded a total of `r metric("FLEET_UTILIZATION_RATE","TNS","last_value_fmt")` in fleet utilization rate for TNS trips. Compared with `r prev_label`, the taxi fleet utilization rate  for TNS trips was `r metric("FLEET_UTILIZATION_RATE","TNS","yoy_fmt")` (YoY). Year-to-date through r end_label, the taxi fleet utilization rate  for TNS trips was `r metric("FLEET_UTILIZATION_RATE","TNS","ytd_yoy_fmt")` relative to the same period in `r prev_year`. On a month-over-month (MoM) basis, TNS fleet utilization changed by `r metric("FLEET_UTILIZATION_RATE","TNS","mom_fmt")`. `r trend_narrative("FLEET_UTILIZATION_RATE", "TNS")` `r shift_narrative("FLEET_UTILIZATION_RATE", "TNS")`
