...
where \|∙\| counts the number of elements in the set.
...
$$
TR_{j,t} = \sum_{i \in T_{j,t}} r_i\tag{2}
$$ 
...
$$
WAV^{1}_{j,t} = \bigl\lvert R_{j,t} \cap V_{j,t} \bigr\rvert\tag{3}
$$ 
...
$$
WAV^{2}_{j,t} = \bigl\lvert R_{j,t} \cap V_{j,t} \cap A_{j,t} \bigr\rvert\tag{4}
$$ 
...
$$
WAV^{\text{per1}}_{j,t}
= \frac{WAV^{1}_{j,t}}{P_{j,t}} \times 1000\tag{5}
$$ 
...
$$
WAV^{\text{per2}}_{j,t}
= \frac{WAV^{2}_{j,t}}{P_{j,t}} \times 1000 \tag{6}
$$
...
= \frac{\lvert V_{j,t} \rvert}{P_{j,t}} \times 1000\tag{7}
$$
...
$$
HHI_{j,t} = \sum_{i=1}^{n} s_{i,j,t}^{2}\tag{10}
$$  
...
$$
T = \sum_{i=1}^{n}
\left( \frac{x_i}{\mu} \ln\!\left(\frac{x_i}{\mu}\right) \right)\tag{11}
$$ 
...
$$
RevPerVehicle_{j,t}
= \frac{TR_{j,t}}{\lvert V_{j,t} \rvert}\tag{13}
$$
...
= \frac{AV_{i,t}}{Capacity_{i,t}}\tag{14}
$$ 
...
$$
VOR_{i,t} = \frac{D_{i,t}}{L_{i,t}}\tag{15}
$$ 
...
= \min_i \left( t_{\text{assign},i}^{\text{vehicle}} \right)\tag{16}
$$
...
= \max_i \left( t_{\text{arrive},i}^{\text{dropoff}} \right)\tag{17}
$$
...
$$
VORPR = \frac{D_{i,t}}{O_{i,t}}\tag{21}
$$
...
- t_{\text{assign},i}^{\text{vehicle}}\tag{22}
$$
...
$$
DR_{j,t}
= \bigl\lvert \{ dr_i \mid i \in T_{j,t} \} \bigr\rvert\tag{24}
$$ 
here:
...
*`r fig_caption(paste(region_name, "Identification of Peak Hours"))`*

```{r}
src2 <- "\\\\Sfp.idir.bcgov\\s143\\S86501\\PTBoard\\Economics\\templates\\peak.png"

dir.create("img-cache", showWarnings = FALSE)
dst2 <- file.path("img-cache", "peak.png")

if (!file.exists(dst2)) {
  ok2 <- file.copy(src2, dst2, overwrite = TRUE)
  if (!ok2) stop("Could not copy peak image from network share")
}

knitr::include_graphics(dst2)
...
$$
PeakVOR_{i,t}
= \frac{D^{\text{peak}}_{i,t}}{L^{\text{peak}}_{i,t}}\tag{25}
$$ 
...
### New Vehicle Occupancy Rate 
*`r fig_caption(paste(region_name, "New York City VOR (2023 and 2024)"))`*
```{r}
