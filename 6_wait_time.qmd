...
make_wait_time_percentile_chart <- function(
  ...
  if (!is.null(date_from)) df <- df %>% filter(date >= date_from)
  if (!is.null(date_to))   df <- df %>% filter(date <= date_to)

  if (nrow(df) == 0) stop("No percentile data found.")

  # آخرین ماه
  last_date <- max(df$date, na.rm = TRUE)
  df <- df %>% filter(date == last_date)

  # percentile عددی از اسم indicator
  df <- df %>%
    mutate(
      percentile = as.numeric(stringr::str_extract(indicator, "\\d+")),
      service = factor(service, levels = services)
    ) %>%
    group_by(service, percentile) %>%
    summarise(value = mean(value, na.rm = TRUE), .groups = "drop")

  ggplot(df, aes(x = percentile, y = value, color = service)) +
